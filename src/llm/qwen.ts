/**
 * @fileoverview Alibaba Qwen adapter implementing {@link LLMAdapter}.
 *
 * Qwen models are accessible via DashScope, which provides an
 * OpenAI-compatible API endpoint. This adapter uses the `openai` SDK
 * with a custom base URL.
 *
 * Models:
 *   - `qwen2.5-coder-32b-instruct` — Boilerplate + scaffolding ($0.66/$0.90)
 *   - `qwen3-30b-a3b`              — MoE debugging ($0.66/$0.90)
 *
 * API key resolution:
...
// ---------------------------------------------------------------------------
// Internal helpers — reuse from DeepSeek pattern (OpenAI-compatible)
// ---------------------------------------------------------------------------
...
export class QwenAdapter implements LLMAdapter {
  ...
  constructor(apiKey?: string) {
    ...
    if (!key) {
      throw new Error(
        'DashScope API key not found. Set DASHSCOPE_API_KEY in your .env file or pass it to the constructor.',
      )
    }
    this.client = new OpenAI({ apiKey: key, baseURL: DASHSCOPE_BASE_URL })
  }
